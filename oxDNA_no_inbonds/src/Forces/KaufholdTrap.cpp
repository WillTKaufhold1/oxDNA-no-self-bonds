...
template <typename number>
void KaufholdTrap<number>::init (BaseParticle<number> ** particles, int N, number * my_box_side_ptr){
	...
	OX_LOG (Logger::LOG_INFO, "Adding KaufholdTrap (stiff=%g, r0=%g, ref_particle=%d, PBC=%d on particle %d", this->_stiff, this->_r0, _ref_id, PBC, _particle);
...
LR_vector<number> KaufholdTrap<number>::value (llint step, LR_vector<number> &pos) {
    ...
    if (pow(dr.module() - _r0,2) < 1){
	    return (-dr / dr.module()) * (dr.module() - _r0) * this->_stiff;
    }
    else {
	    return (-dr / dr.module()) * (dr.module() - _r0) * 0;
    }
}
